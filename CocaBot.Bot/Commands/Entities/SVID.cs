            ...
            if (Inputname != null)
            {
                ...
                if (svids != null)
                {
                    if (svids.Count == 1)
                    {
                        ...
                        await ctx.Channel.SendMessageAsync($"{svid.Key}'s SpookVooper Name: {svid.Value}").ConfigureAwait(false);
                    }
                    else
                    {
                        string msgCont = null;
                        ...
                        await ctx.RespondAsync($"List of SVID's: {msgCont}").ConfigureAwait(false);
                    }
                }
                else
                {
